...
def findderivativeandupdate(X_train, Y_train,Theta,alpha):
    ...
    for i in range(len(X_train)):
        N=float(len(X_train))
        Predictedvalue= 1.0/(1.0+np.exp(Theta[0] + Theta[1]*X_train))
        Actualvalue= Y_train
        

        factor1= (-1/N) * sum(X_train*(Actualvalue-Predictedvalue))
        factor2= (-1/N) * sum(Actualvalue-Predictedvalue)     
        sum1=  factor1
        sum2=  factor2
...
def findtrainingerror(X_train,Y_train,Theta):
    ...
    for i in range(len(X_train)): 
        ...
        predictedvalue= 1.0 / (1.0 +np.exp(Theta[0] + Theta[1]*X_train[i]))
        actualvalue= Y_train[i]        
        # COST FUNCTION (J)= (PREDICTED VALUE - ORIGINAL VALUE)^2        
        
        factor= actualvalue*np.log(predictedvalue) + (1.0-actualvalue)*np.log(1.0-predictedvalue)
        #(actualvalue-predictedvalue)
        sum1= sum1 + factor
    costfunction=sum1/len(X_train)
    train_error= costfunction
    return (train_error)  
